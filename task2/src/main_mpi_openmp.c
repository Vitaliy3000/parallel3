...
void calc_A(double *arr, double *new_arr, double *x, double *y, struct Neighbors *neighbors) {
    ...
    #pragma omp parallel for private(arr_ij, arr_i_p_j, arr_i_n_j, arr_i_j_p, arr_i_j_n)
    for (int i = 0; i < m; i++) {
        for (int j = 0; j < n; j++) {
            arr_ij = arr[i*n+j];
...
void calc_B(double *arr, double *new_arr, double *x, double *y, struct Neighbors *neighbors) {
    ...
    #pragma omp parallel for private(arr_i_p_j, arr_i_n_j, arr_i_j_p, arr_i_j_n)
    for (int i = 0; i < m; i++) {
        for (int j = 0; j < n; j++) {
...
void update(double *arr, struct Neighbors *neighbors) {
    ...
    if (neighbors->bottom_proc_num != -1) {
        ...
        for (int i = 0; i < n; i++)
            neighbors->to_bottom_neighbor_values[i] = arr[(m-1)*n + i];
    ...
    if (neighbors->top_proc_num != -1) {
        #pragma omp parallel for
        for (int i = 0; i < n; i++)
            neighbors->to_top_neighbor_values[i] = arr[i];
    ...
    if (neighbors->right_proc_num != -1) {
        #pragma omp parallel for
        for (int i = 0; i < m; i++)
            neighbors->to_right_neighbor_values[i] = arr[i*n + n-1];
    ...
    if (neighbors->left_proc_num != -1) {
        #pragma omp parallel for
        for (int i = 0; i < m; i++)
            neighbors->to_left_neighbor_values[i] = arr[i*n];
...
int solve(double *omega, double *x, double *y, double eps, struct Neighbors *neighbors) {
    ...
    for (int k = 0; k < MAX_ITER; k++) {
        ...
        #pragma omp parallel for
        for (int i = 0; i <  neighbors->count_n * neighbors->count_m; i++)
            omega[i] += r[i];
    }
...
int main(int argc, char *argv[]) {
    ...
    // init omega
    #pragma omp parallel for
    for (int i = 0; i < neighbors.count_m * neighbors.count_n; i++)
        omega[i] = 1;

    // init x, y
    #pragma omp parallel for
    for (int i = 0; i < neighbors.count_m; i++)
        x[i] = MIN_X + (displs_m[neighbors.coord_m]+1+i)*h1;

    #pragma omp parallel for
    for (int i = 0; i < neighbors.count_n; i++)
        y[i] = MIN_Y + (displs_n[neighbors.coord_n]+1+i)*h2;
    ...
    // if (rank == root) {
    //     FILE *fp;
    //     fp = fopen("./results/parallel_mpi_openmp_160_160_1", "w");
    //     if (fp == NULL) {
    //         perror("open failed");
    //         exit(1);
    //     }
